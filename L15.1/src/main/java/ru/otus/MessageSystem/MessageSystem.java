...
public class MessageSystem {
    ...
    public void startThread(Addressee addressee) {
        Optional<Map.Entry<Address, Addressee>> entryOptional = addresseeMap.entrySet().stream()
                .filter(entry -> entry.getValue().equals(addressee)).findFirst();
        if (entryOptional.isPresent()) {
            Map.Entry<Address, Addressee> entry = entryOptional.get();
